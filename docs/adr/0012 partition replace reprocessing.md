...
**Discovered characteristic (2026-08-25): duplicate rate varies enormously by month —** 35.4% in December 2010 (the dataset's first month) versus 0.70% and 1.55% in January and November 2011. Investigated and confirmed genuine at the row level (identical invoice, stock code, quantity, timestamp to the millisecond, price). December 2010 is treated as a documented exception in the duplicate-rate test rather than absorbed into a wider "normal" threshold, since a range wide enough to cover it would no longer catch a real anomaly elsewhere. The reconciliation chain does not and cannot catch this kind of issue — Glue counts duplicate rows as rows, so every count agrees regardless. This is the boundary of what row-count reconciliation proves.